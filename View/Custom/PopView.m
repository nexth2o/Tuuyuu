...
+ (PopView *)sharedInstance {
    static PopView *sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedClient = [[PopView alloc]init];
    });
    return sharedClient;
}
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([PopView sharedInstance].shadeBackgroundType == ShadeBackgroundTypeSolid) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([PopView sharedInstance].tapOutsideToDismiss) {
        [[PopView sharedInstance] hideAnimated:YES withCompletionBlock:nil];
    }
}
